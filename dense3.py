...
def plot_history(history, x, y):
    ...
    plt.ylabel('FDDT log10(q/V) calculado')
    plt.legend()
    plt.show()
...
outputs_train = dataall.iloc[:quanti, 15].values
outputs_test = dataall.iloc[quanti:2291, 15].values
...
fddt_log_q_v = dataall.iloc[2291:, 15]
respostas = pd.DataFrame(
    {'Predictions': preditcions, 'fddt_log_q_v': fddt_log_q_v})

correlacao = respostas['fddt_log_q_v'].corr(respostas['Predictions'])
print(correlacao)

plot_history(history, respostas['Predictions'], respostas['fddt_log_q_v'])
